...
K \cdot \left(N_{0} - N\right) + D \cdot \delta^{N}_{D} -\sum_{i=1}^{n_p} [\mu_i \cdot U_{i}(N_0,Si_0)\cdot L_i(PAR)\cdot T_i(SST) \cdot P_{i}] 
\nonumber  \\
...
\mu_{dt} \cdot U_{dt}(N_0,Si_0)\cdot L_{dt}(PAR)\cdot T_{dt}(SST) \cdot P_{dt}
- m_{dt} \cdot P_{dt}
- \sum_{j} [I^{tot}_j \frac{p_{dt,j} \cdot P_{dt}} {R_{j}} Z_{j}]
\nonumber \\
\nonumber \\
...
\sum_{j} [(1-\delta_Z) I^{tot}_j \cdot Z_{j}] +
\sum_{j} [m_j \cdot Z_{j}] +
\sum_{i} [m_i \cdot P_{i}] -
K \cdot D -
D \cdot \delta^{N}_{D}
\nonumber
\end{eqnarray}
...
where:\\
\mbox{} \hspace{.5cm} $N_0=$ Nitrogen concentration below below mixed layer [$\mu M$],\\
\mbox{} \hspace{.5cm} $N=$ Integrated nitrogen concentration above mixed layer [$\mu M$],\\
...
\mbox{} \hspace{.5cm} $I^{tot}_j= \mu^{Z}_j⋅\frac{R_{j}}{R_{j} + k_{Z}}$ Total intake XXX $i$,\\
\mbox{} \hspace{.5cm} $R_{j}= \sum_{i} (p_{i j}⋅P_{i})$ Total intake XXX $i$,\\
...
\iffalse % HERE BEGINNING OF COMMENT
