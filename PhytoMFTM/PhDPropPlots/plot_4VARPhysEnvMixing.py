...
from PhDPropPlots.runModel_4VARPhysEnvMixing import out5P2Z, out5P2Z_2, out5P2Z_3, out5P2Z_4, timedays_model #, out5P2Zconstant
...
def plotMODELFORCINGoutput(outarray, outarray2, outarray3, outarray4, pfn, zn):
    f2, (ax1, ax2, ax3, ax4, ax5) = plt.subplots(5, 4, sharex='col')
    # N / Si / Pdt / Pc / Pdn / Pn / Zmu / Zlambda / D
    # PLOTTING
    timedays = timedays_model#[1:366]
    # truncate outarraySiNO to last year of 5:
    ...
    outarray_ly_4 = outarray4#[1460:1825]
    ...
    # Figure 4
    ax1[3].set_title('MLD and stochastic', fontsize=10)
    ax1[3].plot(timedays, outarray_ly_4[:, 3 + zn + pfn], c="Ni", lw=lws[0], alpha=alphas[0], label='Model')  # MLD
    ax1[3].set_ylim(0, 101)
    ax1[3].invert_yaxis()
    #ax1[2].set_ylabel('MLD \n' '[m]', multialignment='center', fontsize=10)

    # subplot 2
    ax2[3].plot(timedays, outarray_ly_4[:, 3 + zn + pfn + 5], c="Ni", lw=lws[0], alpha=alphas[0], label='N0')  # MLD
    ax2[3].set_ylim(0, 10)
    #ax2[2].set_ylabel('$N_0$ \n' '[µM]', multialignment='center', fontsize=10)

    # Si
    ax3[3].plot(timedays, outarray_ly_4[:, 3 + zn + pfn + 2], c="Ni", lw=lws[0], alpha=alphas[0], label='K')  # K
    ax324 = ax3[3].twinx()
    ax324.plot(timedays, outarray_ly_4[:, 3 + zn + pfn + 4], c="MLD", lw=lws[0], alpha=alphas[0], label='U')  # U
    ax324.set_ylabel('U \n' '[$d^{-1}$]', multialignment='center', fontsize=10)
    #ax3[3].set_ylim(-0.1, 0.02)
    #ax324.set_ylim(-0.1, 0.02)
    ax324.yaxis.label.set_color('MLD')
    ax3[3].plot(0, 0, c="MLD", lw=lws[0], alpha=alphas[0], label='U')  #
    #ax3[2].set_ylabel('K  \n' '[$d^{-1}$]', multialignment='center', fontsize=10)
    ax3[3].legend(loc='lower right', fontsize=6)

    ax4[3].plot(timedays, outarray_ly_4[:, 3 + zn + pfn + 1], c="Ni", lw=lws[0], alpha=alphas[0])  # NMixing
    ax4[3].set_ylim(-0.01, 0.02)
    #ax4[2].set_ylabel('NMixing \n' '[µM $d^{-1}$]', multialignment='center', fontsize=10)

    ax5[3].plot(timedays, outarray_ly_4[:, 3 + zn + pfn + 6], c="Ni", lw=lws[0], alpha=alphas[0])  # Phy GAINS
    #ax5[2].set_ylabel('N \n' '[µM]', multialignment='center', fontsize=10)
    ax5[3].set_ylim(0, 10)
    ax5[3].set_xlabel('Day in year', fontsize=10)

    plt.margins(x=0)

    ##############################################################################################################
    # Figure 3
    ax1[2].set_title('MLD driven', fontsize=10)
    ...
    #ax322.set_ylabel('U \n' '[$d^{-1}$]', multialignment='center', fontsize=10)
    #ax323.set_ylim(-0.1, 0.02)
    #ax3[2].set_ylim(-0.1, 0.02)
    ...
    # Figure 2
    ax1[1].set_title('stochastic K', fontsize=10)
    ...
    #ax322.set_ylabel('U \n' '[$d^{-1}$]', multialignment='center', fontsize=10)
    #ax322.set_ylim(-0.1, 0.02)
    #ax3[1].set_ylim(-0.1, 0.02)
    ...
    # Figure 1
    ax1[0].set_title('constant K', fontsize=10)
    ...
    #ax32.set_ylabel('U \n' '[$d^{-1}$]', multialignment='center', fontsize=10)
    #ax32.set_ylim(-0.1, 0.02)
    #ax3[0].set_ylim(-0.1, 0.02)
...
plotMODELFORCINGoutput(out5P2Z, out5P2Z_2, out5P2Z_3, out5P2Z_4, 5, 2)