...
def plotoutput(outarray, pfn, zn, i_plot, title):
    ...
    ax1[i_plot].scatter(np.arange(1, 13, 1) * 30 - 15, fx.verif.N, label='WOA data')
    ...
    ax1[i_plot].legend(fontsize='x-small')
    # Si
    ...
    if ChlConv == False:
        ...
        ax2s.scatter(np.arange(1,13,1) * 30 -15, fx.verif.chla)
        ax2[i_plot].scatter(0,0, label='MODISaq data')
        ...
        # modeldepth = 100
        # molarmasschl = 893.51  # g/mol
        CtoChla = 40  # g/g
        MolarMassC = 12.0107
        CtoNratioPhyto = 6.625
        muMolartoChlaconvfactor = CtoChla / MolarMassC / CtoNratioPhyto  # Chla as mg/m-3 to

        ax2[i_plot].scatter(np.arange(1, 13, 1) * 30 - 15, np.array(fx.verif.chla) * muMolartoChlaconvfactor, label='MODISaq data')


    Pall = [outarray_ly[:, 3 + zn + i] for i in range(pfn)]
    P_Max = np.max(Pall) + 0.5 * np.max(Pall)

    ax2[i_plot].plot(timedays, sum([outarray_ly[:, 3 + zn + i] for i in range(pfn)]), c=colors[4], lw=lws[1], label='Model')
    ax2[i_plot].legend(fontsize='x-small')
