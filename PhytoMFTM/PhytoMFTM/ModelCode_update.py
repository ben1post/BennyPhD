...
def new_way_of_creating_model(x,t,modelsetup):
    ...
    ZLosses = z.interzoograze(Gj, Z) + z.mixing(K_Z) + ZMortality + z.higherorderpred(Z)

    # Detritus Fluxes
    DGains = sum(z.unassimilatedgrazing(ZooFeeding)) + sum(ZMortality) + sum(PMortality)
    DLosses = sum(d.remineralisation(D)) - D * K

    ## THESE ALL HAVE TO BE np.arrays, otherwise this won't work!

    Px = PGains - PhytoLosses

    Nx = d.remineralisation(D) + n.mixing(N0, N) - sum(PGains)  # Nutrient draw down
