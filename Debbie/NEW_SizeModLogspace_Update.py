...
from NEW_SizeMod_EnvForc import SizeMod_EnvForc
...
class SizeModLogspace:
    ...
    """
    #mld, par, sst, dmdt,
    def __init__(self, freq, minP=0., maxP=2., numP=3, minZ=5, maxZ=200, numZ=2, numYears=5,
        ...
        :param mixfreq: 'constant', 'medium', 'high'
        :param minP:
        :param maxP:
        :param numP:
        :param minZ:
        ...
        """
        self.freq = freq
        self.mld = SizeMod_EnvForc(fx_type='MLD', freq=self.freq)
        self.par = SizeMod_EnvForc(fx_type='PAR')       # photosynethically activated radiation (E*m^-2*d^-1)
        self.sst = SizeMod_EnvForc(fx_type='LWST')      # sea surface temperature/lake surface water temperature (LSWT)
        self.dmdt = SizeMod_EnvForc(fx_type='dMdt', freq=self.freq)        # the derivative of MLD
        ...
        self.t_array = np.arange(0, 365 * self.numYears, 1.0)  # Time array
        ...
        self.solution = odeint(self.AlloNPsZD, self.init_con, self.t_array, rtol=1.e-12, atol=1.e-12, hmax=1.e16,
                               hmin=1.e-20)
    ...
    def temp(self, timearr):
        """
        Eppley's formulation
        """
        return np.exp(0.063 * timearr)
    ...
    def mixing(self, timearr):
        """k refers to the cross thermocline mixing factor"""
        return (self.k + max(self.dmdt.return_deriv(timearr), 0.)) / self.mld.return_intrp(timearr)
    ...
    def AlloNPsZD(self, y, t):
        ...
        var[1] = (- self.Mz * Zsmall  # Natural mortality Z1
                  ...
                  - (self.dmdt.return_deriv(t) / self.mld.return_intrp(t)) * Zsmall)  # Mixing loss Z1

        # [dZ2dt]
        var[2] = (- self.Mz * Zlarge  # Natural mortality Z2
                  - self.Mz2 * (Zlarge ** 2)  # Top predator grazing Z2
                  - (self.dmdt.return_deriv(t) / self.mld.return_intrp(t)) * Zlarge)  # Mixing loss Z2
        ...
        for i in range(len(Ps)):
            ...
            # [dNdt]
            var[0] = (var[0]
                      - self.Allo_mumax(Psize) * (self.temp(self.sst.return_intrp(t)) * self.Allo_NU(N, Psize) *
                                                  self.LightLim(self.par.return_intrp(t), self.mld.return_intrp(t))) * P
            ...
            var[4 + i] = (var[4 + i]
                          + self.Allo_mumax(Psize) * (self.temp(self.sst.return_intrp(t)) * self.Allo_NU(N, Psize)
                                                      * self.LightLim(self.par.return_intrp(t),
                                                                      self.mld.return_intrp(t))) * P  # Uptk
