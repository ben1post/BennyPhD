...
from SizeModEnvForc_v2_rmt import SizeModEnvForc
...
class SizeModLogspace:
    ...
    def __init__(self, freq, minP=0., maxP=2., numP=3, minZ=5, maxZ=200, numZ=2, numYears=5,
        ...
        :param SizeTol:
        ----------
        MLD = Mixed layer depth (meter)
        PAR = photosynethically activated radiation (E*m^-2*d^-1)
        SST = lake surface water temperature
        dMdt =  the derivative of MLD (rate of change of MLD)
        """
        self.freq = freq
        self.mld = SizeModEnvForc(fx_type='MLD', freq=self.freq)
        self.par = SizeModEnvForc(fx_type='PAR', freq=None)
        self.sst = SizeModEnvForc(fx_type='LWST', freq=None)
        self.dmdt = SizeModEnvForc(fx_type='dMdt', freq=self.freq)
        ...
        self.solution = odeint(self.AlloNPsZD, self.init_con, self.t_array, full_output=True)
    ...
    def mixing(self, timearr):
        ...
        return (self.k + max(self.dmdt.return_forcing(timearr), 0.))\
               / self.mld.return_forcing(timearr)
    ...
    def AlloNPsZD(self, y, t):
        ...
        var[1] = (- self.Mz * Zsmall  # Natural mortality Z1
                  ...
                  - (self.dmdt.return_forcing(t) / self.mld.return_forcing(t)) * Zsmall)  # Mixing loss Z1

        # [dZ2dt]
        var[2] = (- self.Mz * Zlarge  # Natural mortality Z2
                  - self.Mz2 * (Zlarge ** 2)  # Top predator grazing Z2
                  - (self.dmdt.return_forcing(t) / self.mld.return_forcing(t)) * Zlarge)  # Mixing loss Z2
        ...
        for i in range(len(Ps)):
            ...
            # [dNdt]
            var[0] = (var[0]
                      - self.Allo_mumax(Psize) * (self.temp(self.sst.return_forcing(t)) * self.Allo_NU(N, Psize) *
                                                  self.LightLim(self.par.return_forcing(t),
                                                                self.mld.return_forcing(t))) * P
            ...
            var[4 + i] = (var[4 + i]
                          + self.Allo_mumax(Psize) * (self.temp(self.sst.return_forcing(t)) * self.Allo_NU(N, Psize) *
                                                      self.LightLim(self.par.return_forcing(t),
                                                                    self.mld.return_forcing(t))) * P  # Uptk
