...
from SizeModEnvForc_v2_rmt_old import SizeModEnvForc
...
class SizeModLogspace:
    ...
    def mixing(self, timearr):
        ...
        return (self.k + max(self.dmdt.intrp_forcing(timearr), 0.))\
               / self.mld.intrp_forcing(timearr)
    ...
    def AlloNPsZD(self, y, t):
        ...
        var[1] = (- self.Mz * Zsmall  # Natural mortality Z1
                  ...
                  - (self.dmdt.intrp_forcing(t) / self.mld.intrp_forcing(t)) * Zsmall)  # Mixing loss Z1

        # [dZ2dt]
        var[2] = (- self.Mz * Zlarge  # Natural mortality Z2
                  - self.Mz2 * (Zlarge ** 2)  # Top predator grazing Z2
                  - (self.dmdt.intrp_forcing(t) / self.mld.intrp_forcing(t)) * Zlarge)  # Mixing loss Z2
        ...
        for i in range(len(Ps)):
            ...
            # [dNdt]
            var[0] = (var[0]
                      - self.Allo_mumax(Psize) * (self.temp(self.sst.intrp_forcing(t)) * self.Allo_NU(N, Psize) *
                                                  self.LightLim(self.par.intrp_forcing(t),
                                                                self.mld.intrp_forcing(t))) * P
            ...
            var[4 + i] = (var[4 + i]
                          + self.Allo_mumax(Psize) * (self.temp(self.sst.intrp_forcing(t)) * self.Allo_NU(N, Psize) *
                                                      self.LightLim(self.par.intrp_forcing(t),
                                                                    self.mld.intrp_forcing(t))) * P  # Uptk
