...
class SizeModEnvForc:
    ...
    def __init__(self, fx_type, freq=None, Zmix=80, ThermoZ=2.5):
        ...
        self.interpolated_forcing = self.intrp_forcing()
    ...
    def mld_func(self, tstep):
        if self.fx_type == 'MLD': 
            if self.freq == 'constant':
                return self.interpolated_forcing(tstep)
            elif self.freq == 'medium':
                out = (-((self.Zmix - self.ThermoZ) / 2 + self.ThermoZ) + (self.Zmix - self.ThermoZ) / 2 *
                             np.cos(tstep / 14.525)) * -1
            elif self.freq == 'high':
                out = (-((self.Zmix - self.ThermoZ) / 2 + self.ThermoZ) + (self.Zmix - self.ThermoZ) / 2 *
                             np.cos(tstep / 4.825)) * -1
        elif self.fx_type == 'dMdt':                
            if self.freq == 'constant':
                return self.interpolated_forcing(tstep)
            elif self.freq == 'medium':
                out = 1 * (-self.ThermoZ + self.Zmix) * np.sin(tstep / 14.525) / (2 * 14.525)
                # derivative function obtained from sympy
            elif self.freq == 'high':
                out = 1 * (-self.ThermoZ + self.Zmix) * np.sin(tstep / 4.825) / (2 * 4.825)
        return out

    def intrp_forcing(self):
        """
